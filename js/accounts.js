...
function renderAccChart(){
  ...
  chartAcc=new Chart(ctx,{type:'line',data:{labels:history.map(h=>h.label),datasets:[{label:'Zůstatky (Kč)',data:history.map(h=>h.value),borderColor:'#4f8ef7',backgroundColor:'rgba(79,142,247,0.1)',borderWidth:2,pointRadius:4,pointBackgroundColor:'#4f8ef7',fill:true,tension:0.35}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:v=>v.raw.toLocaleString('cs-CZ',{minimumFractionDigits:2,maximumFractionDigits:2})+' Kč'}}},scales:{x:{ticks:{color:'#8b92a8',font:{size:11},maxRotation:45,autoSkip:true},grid:{color:'rgba(255,255,255,0.05)'}},y:{ticks:{color:'#8b92a8',font:{size:11},callback:v=>v.toLocaleString('cs-CZ',{minimumFractionDigits:2,maximumFractionDigits:2})},grid:{color:'rgba(255,255,255,0.05)'}}}}});
}
