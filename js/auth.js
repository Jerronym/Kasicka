...
// ── Auto-sync mezi zařízeními (stále otevřená karta drží čerstvá data) ──
// Řeší případ, kdy je karta v prohlížeči otevřená dlouho a pracuje se zastaralým
// snapshotem: jiné zařízení mezitím uložilo novější data. Bez tohohle by uložení
// vyvolalo konflikt (saveToCloud → "Data změněna z jiného zařízení. Přepsat?").
// Teď si karta čerstvá data sama stáhne dřív, než stihne vzniknout konflikt.

// Stáhne aktuální data z cloudu do otevřené karty a překreslí.
async function reloadFromCloud(){
  ...
  try{
    const res=await supa.from('user_data').select('data,updated_at').eq('user_id',currentUser.id).single();
    data=res.data;
  }catch(e){ return; /* offline — zkusí se příště */ }
  if(!data) return;
  applyImport(data.data||{});
  _lastCloudSave=data.updated_at||_lastCloudSave;
  clearPendingSync();
  processRecurringTxns();
  initCategories();
  refreshCatSelect();
  ...
  updatePeriodNavLabel();
  markDirty();
  if(typeof toast==='function') toast('Data aktualizována z jiného zařízení.','info');
}

// Ověří, zda v cloudu není novější verze, a případně ji načte.
// Hlídá, aby uživatele nevyrušil uprostřed editace ani nepřepsal
// nesynchronizované offline změny (pending sync).
async function checkCloudFreshness(){
  if(!currentUser || demoMode || document.hidden) return;
  if(hasPendingSync()) return;                              // neuložené lokální změny → nepřepisuj
  if(document.querySelector('[id^="modal-"].open')) return; // uživatel právě edituje ve formuláři
  try{
    ...
    if(_lastCloudSave && data.updated_at!==_lastCloudSave) await reloadFromCloud();
    else if(!_lastCloudSave) _lastCloudSave=data.updated_at;
  }catch(e){/* offline — zkusí se příště */}
}

// Spouštěče: návrat na kartu, fokus okna, obnova připojení a periodicky každých 30 s.
