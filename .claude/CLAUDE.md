...
- **supabase-js v2 Web Locks hang on mobile/PWA**: `onAuthStateChange` holds an auth lock while the callback runs. Any `supa.from()` call inside the callback (even indirectly via `showApp()`) tries to re-acquire the same lock → deadlock. Fix 1: wrap callback body in `setTimeout(0)` so the lock is released first. Fix 2: pass a no-op `lock` to `createClient` (`lock: async (_n,_t,fn)=>await fn()`) — eliminates Web Locks entirely. This is safe for single-user apps and also fixes the PWA freeze where Android suspends background contexts without releasing the lock, causing all subsequent queries to hang forever.
